...
export default function AdminTestimonialsPage() {
  ...
  const save = async () => {
    ...
    const url = isEdit ? `${API}/api/admin/testimonials/${form.id}/` : `${API}/api/admin/testimonials/`;
  ...
  const deleteTestimonial = async (id: number) => {
    try {
      await fetch(`${API}/api/admin/testimonials/${id}/`, { method: "DELETE" });
      setDeleteId(null);
      fetchTestimonials();
    } catch {
      ...
      <main className="container mx-auto px-6 max-w-4xl pt-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <MessageSquare className="h-6 w-6 text-primary" /> Testimonials
          </h1>
          <button
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200">
            ...
            <button onClick={() => setForm(emptyTestimonial())} className="mt-4 text-primary font-bold text-sm hover:underline">
          ...
          <div className="space-y-4">
            {testimonials.map(t => (
              <div key={t.id} className={`bg-white rounded-2xl border shadow-sm p-6 flex gap-4 items-start ${t.is_active ? 'border-gray-100' : 'border-gray-100 opacity-60'}`}>
                <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center font-black text-primary flex-shrink-0 text-lg">
                  {t.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-black text-gray-900">{t.name}</span>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${t.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 overflow-y-auto">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-[2rem] shadow-2xl my-8">
            <div className="flex items-center justify-between p-8 border-b border-gray-100">
              <h2 className="text-xl font-black text-gray-900">{form.id ? "Edit Testimonial" : "New Testimonial"}</h2>
            ...
            <div className="p-8 space-y-5">
              ...
              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Quote</label>
                <textarea
                  value={form.quote}
                  onChange={e => setForm(f => f ? { ...f, quote: e.target.value } : f)}
                  rows={4}
                  placeholder="What did they say about Mama Fresh?"
      ...
      {deleteId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl p-8 shadow-2xl max-w-sm w-full text-center">
            ...
            <h3 className="text-xl font-black text-gray-900 mb-2">Delete this testimonial?</h3>
            <p className="text-gray-500 text-sm mb-6">This cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => deleteTestimonial(deleteId)} className="flex-1 py-3 rounded-xl bg-red-600 text-white font-bold hover:bg-red-700 transition-colors">
                Yes, Delete
              </button>
